...
namespace PhatRobit
{
	public class SpriteBillboard : MonoBehaviour
	{
		...
		void Start()
		{
			cameraDirection = Camera.main.GetComponent<CameraDirection>();
			_t = transform;
		...
		void Update()
		{
			_facing = spriteDirection._facing;
			_sprite.flipX = false;
			...
			_billboard.rotation = Quaternion.LookRotation(new Vector3(0, 0, (cameraDirection.transform.position - transform.position).z), Vector3.up);
